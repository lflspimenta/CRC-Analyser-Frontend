...
export default function AnaliseIA({ resultado, pdfFile }: Props) {
  ...
  const analisar = async () => {
    if (!pdfFile) { setError('PDF não disponível. Carrega novamente o CRC.'); return }
  ...
  const renderMarkdown = (md: string) => {
    return md
      ...
      .map((line, i) => {
        if (line.startsWith('## ')) {
          return <h3 key={i} className="text-stone-200 font-medium text-base mt-6 mb-3">{line.slice(3)}</h3>
        }
        if (line.startsWith('- ')) {
          return <li key={i} className="text-stone-400 text-sm leading-relaxed ml-4 list-disc">{line.slice(2)}</li>
        }
  ...
  return (
    <div className="stagger space-y-4">
      <div>
        <h2 className="text-stone-200 font-medium mb-1">Análise por IA</h2>
        <p className="text-sm text-stone-500">
          Relatório personalizado gerado pelo Claude com base no teu CRC real.
        </p>
      </div>
      ...
      {!texto && !loading && (
        <div className="rounded-xl p-8 text-center"
          ...
          <p className="text-stone-300 text-sm mb-1 font-medium">Análise financeira personalizada</p>
          <p className="text-stone-600 text-xs mb-5">
            O Claude analisa os teus 13 contratos e gera um relatório com plano de acção concreto.
            Demora cerca de 15-20 segundos.
          </p>
          {error && (
            <p className="text-red-400 text-xs mb-4 px-4 py-2 rounded-lg"
              style={{ background: 'rgba(239,68,68,0.1)' }}>{error}</p>
          )}
      ...
      {(loading || texto) && (
        <div className="rounded-xl p-5"
          ...
          <div ref={containerRef} className="max-h-[500px] overflow-y-auto space-y-1 pr-2">
            ...
            {loading && (
              <span className="inline-block w-2 h-4 rounded-sm animate-pulse ml-1"
          ...
          {done && (
            <div className="flex gap-3 mt-5 pt-4" style={{ borderTop: '1px solid var(--border)' }}>
              <button onClick={analisar}
                className="text-xs px-3 py-1.5 rounded-lg border transition-colors text-stone-400 hover:text-stone-200"
              ...
              <button
                onClick={() => {
                  ...
                  a.href = url; a.download = 'analise-crc.txt'; a.click()
                }}
                className="text-xs px-3 py-1.5 rounded-lg border transition-colors text-stone-400 hover:text-stone-200"
                style={{ borderColor: 'var(--border)' }}>
                Guardar texto
